...
RUN apt-get update && apt-get install -y --no-install-recommends curl ca-certificates tar clang libpq-dev && rm -rf /var/lib/apt/lists/*
...
WORKDIR /app
RUN install -m 0755 /tmp/nolc/nolc /usr/local/bin/nolc && for vue in accueil login inscription recherche tableau; do nolc nhtml "$vue.nhtml"; done && nolc build main.nol -o nolcore --lien pq --chemin-lib /usr/lib/x86_64-linux-gnu
FROM ubuntu:24.04
